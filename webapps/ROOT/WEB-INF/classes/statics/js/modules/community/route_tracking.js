var vm = new Vue({
    ...
    methods:{
        ...
        reqItemOrign(id){
            ...
            $.ajax({
                type: "POST",
                url: '/firstClass/info/' + id.toString(),
                contentType: "application/json",
                dataType: "json",
                success: function(res){
                    if(res.code == 200){
        ...
        saveVtalkForm (formName,type){
            ...
            self.$refs[formName].validate((valid) =>{
                if (valid) {
                    //如果保存时权重为'',变成'-1'
                    if (self.creatEditForm.weight.toString().trim() == '') {
                        self.creatEditForm.weight = '-1'
                    }
                    //去掉标题头尾空格
                    self.creatEditForm.name = self.creatEditForm.name.trim()
                    if (self.showCreatEditDialogTitle == '新建内容') {
                        var reqUrl = '/firstClass/save'
                    } else if (self.showCreatEditDialogTitle == '编辑内容') {
                        var reqUrl = '/firstClass/update'
                    }
                    $.ajax({
                        type: "POST",
                        url: reqUrl,
        ...
        submitVtalk () {
            ...
            var self = this
            console.log(self.creatEditForm.id)
            var data = {
                id: self.creatEditForm.id.toString(),
                status: '0'
            }
            $.ajax({
                type: "POST",
                url: "/firstClass/update",
                contentType: "application/json",
                data: JSON.stringify(data),
                dataType: "json",
                success: function(res){
        ...
        addThisCoverImg (item) {
            ...
            this.creatEditForm.img = item.picUrl
            this.backToEdit()
        },
        //返回编辑页
        backToEdit (){
        ...
        openUrlvTalkPage(item){
            if(item.status == 0){
                window.open(item.link, "newwindow")
            }
        },
        //时间格式转换工具
