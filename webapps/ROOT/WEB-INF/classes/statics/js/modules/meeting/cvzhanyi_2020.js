var vm = new Vue({
    ...
    created () {
        ...
        if (window.location.href.indexOf('chinaventure.com.cn') !== -1 || window.location.href.indexOf('cvmedia.com.cn') !== -1 || window.location.href.indexOf('117.78.28.103') !== -1) {
            console.log('正式环境')
            this.meetingForm.meetingBaseInfoId = '56'//基础信息ID,写死
            this.meetingForm.meetingAgendaId = ''//日程ID,写死
            this.meetingForm.meetingGuestId = ''//嘉宾ID,写死
        } else {
            console.log('测试环境')
            this.meetingForm.meetingBaseInfoId = '41'//基础信息ID,写死
            this.meetingForm.meetingAgendaId = ''//日程ID,写死
            this.meetingForm.meetingGuestId = ''//嘉宾ID,写死
        }
    },
    ...
    methods:{
        ...
        //添加会议计划条目
        addPlanItem () {
            let Lv1Length = this.meetingForm.meetingJson.planContent.length
            if (this.meetingForm.meetingJson.planContent[Lv1Length - 1].planTitle.trim() !== '' && this.meetingForm.meetingJson.planContent[Lv1Length - 1].paragraphText.trim() !== '') {
                this.meetingForm.meetingJson.planContent.push({
                    planTitle:'',
                    paragraphText:''
                })
            } else {
        ...
        delPlanItem(index){
            let Lv1Length = this.meetingForm.meetingJson.planContent.length
            if (Lv1Length >= 2) {
                this.meetingForm.meetingJson.planContent.splice(index, 1); 
            } else {
                this.$message.error('至少保留一个段落')
            }
        },
