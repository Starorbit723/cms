var vm = new Vue({
    ...
    data: {
        ...
        searchForm:{
            picTitle:'',
            picType:'3',//图片分类查询 0封面 1内容  2图位 3广告 4自媒体头像
            picTypeId:'',//所属图位的id
            starTime:'',
            endTime:''
        },
        ...
        tableData: [{
            ...
            picDesc:'',//图片详情
            picUrl:'',//图片地址
            picType: 3,//图片分类 0封面 1内容  2图位 3广告 4自媒体头像
    ...
    methods:{
        ...
        addEditImg (item) {
            ...
            if(item == 0){
                ...
                this.showChildPage = true
                this.creatOrEdit = 1
                this.advImgForm = JSON.parse(JSON.stringify(item))
                this.originUrl = this.advImgForm.picUrl
                console.log('修改图片')
            }
        },
        //封面图改变时
        coverImgFileChange (file,fileList) {
            ...
            fr.onload = function() {
                //先临时显示一下本地图片
                self.advImgForm.picUrl = this.result
            }
        ...
        submitCreatEdit(formName) {
            ...
            self.$refs[formName].validate((valid) => {
                if (valid) {
                        ...
                        $.ajax({
                            ...
                            success: function(res){
                                if(res.code == 200){
                                    ...
                                    self.advImgForm.picUrl = res.url
                                    self.advImgForm.picEditor = getCookie('userId')
                                    console.log('接受到图片改变后的form',self.advImgForm)
                                    self.submitForm()
                                }else{
                    ...
                    //修改，图片发生了改变
                    } else if (self.creatOrEdit == 1 && (self.advImgForm.picUrl !== self.originUrl )) {
                        $.ajax({
                            type: "POST",
                            contentType: false,
                            processData: false,
        ...
        submitForm () {
            var self = this
            self.$refs['advImgForm'].validate((valid) => {
                if (valid) {
                    if (self.creatOrEdit == 0) {
                    ...
                    $.ajax({
                        ...
                        contentType: "application/json",
                        url: reqUrl,
                        data: JSON.stringify(self.advImgForm),
                        dataType: "json",
                        success: function(res){
                            if(res.code == 200){
                                self.$message.success('保存成功');
                                self.startSearch() //列表回显
                                self.closeEditCreatEdit('advImgForm')
                            }else{
                                mapErrorStatus(res)
                                vm.error = true;
                                vm.errorMsg = res.msg;
